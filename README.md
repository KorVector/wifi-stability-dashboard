...
- Supabase 저장에 성공한 튜플만 `DB 측정 기반 사용 추천`과 `DB 기반 AP 우선 후보` 계산에 사용
...
6. `원격 기록 불러오기`를 누르면 Supabase의 기존 튜플을 가져와 시간대별 추천과 AP 후보를 다시 계산
...
웹의 사용 추천과 AP 후보는 정적 더미 파일을 사용하지 않습니다.

- Supabase `wifi_measurements` 테이블에 저장된 튜플만 사용
- 위치별 평균 위험도, 평균 지연, 평균 다운로드 계산
- 시간대별 측정값이 여러 개 있으면 가장 위험도가 낮고 다운로드가 높은 시간대를 추천
- 특정 위치가 여러 번 높게 나오면 AP 설치 우선 후보로 정렬

## AI 분석 기능

AI 분석은 브라우저에서 OpenAI API key를 직접 사용하지 않습니다. Secret key를 `app.js`, `index.html`, GitHub 저장소, 웹사이트 입력칸에 넣으면 안 됩니다.
...
```text
OPENAI_API_KEY = 복사한 OpenAI secret key
```
...
그다음 다시 deploy하면 웹의 `AI 분석` 버튼이 `netlify/functions/analyze.js`를 통해 OpenAI API를 호출합니다. AI에게 전달되는 값은 Supabase에 저장된 측정 튜플과 안정화 기준뿐입니다.
